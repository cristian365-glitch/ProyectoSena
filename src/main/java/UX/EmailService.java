...
/**
 * Servicio de email usando Brevo API (ex-Sendinblue)
 * 300 emails/día GRATIS - SIN dominio necesario - Envía a CUALQUIER email
 * @author Calixto
 */
public class EmailService {
    ...
    static {
        ...
        if (BREVO_API_KEY == null || BREVO_API_KEY.trim().isEmpty()) {
            System.err.println("❌ ERROR: Variable de entorno BREVO_API_KEY no configurada en Render");
        ...
        if (EMAIL_FROM == null || EMAIL_FROM.trim().isEmpty()) {
            ...
            System.err.println("⚠️ Usa el email que verificaste en Brevo");
        }
        
        if (BREVO_API_KEY != null && EMAIL_FROM != null) {
            System.out.println("✅ Variables de entorno de Brevo cargadas correctamente");
            System.out.println("📧 Email configurado: " + EMAIL_FROM);
        }
    }
    
    /**
     * Enviar código de recuperación por email usando Brevo API
     */
    public static boolean enviarCodigoRecuperacion(String emailDestino, String codigo) {
        ...
        if (BREVO_API_KEY == null || EMAIL_FROM == null) {
            ...
            System.err.println("⚠️ Configura BREVO_API_KEY y EMAIL_FROM en Render");
            return false;
        }
        ...
        try {
            ...
            // Crear la URL de la API de Brevo
            ...
            sender.put("name", EMAIL_FROM_NAME);
            sender.put("email", EMAIL_FROM);
            ...
            email.put("htmlContent", crearHTMLEmail(codigo));
            ...
            if (responseCode == 200 || responseCode == 201) {
                ...
                System.out.println("📬 Respuesta Brevo: " + response.toString());
                return true;
            } else {
                // Leer error
                BufferedReader br = new BufferedReader(
                ...
                if (responseCode == 401) {
                    System.err.println("⚠️ SOLUCIÓN: API Key incorrecta. Verifica BREVO_API_KEY en Render");
                } else if (responseCode == 400) {
                    System.err.println("⚠️ SOLUCIÓN: Email FROM no verificado en Brevo");
                    System.err.println("⚠️ Ve a Brevo → Senders → Verifica tu email");
                }
    ...
    public static boolean verificarConfiguracion() {
        System.out.println("🔍 Verificando configuración de Brevo...");
        
        if (BREVO_API_KEY == null || BREVO_API_KEY.trim().isEmpty()) {
            System.err.println("❌ BREVO_API_KEY no configurado");
            return false;
        }
    ...
    public static void main(String[] args) {
        System.out.println("🧪 Probando configuración de Brevo API...");
        ...
        if (!verificarConfiguracion()) {
            ...
            System.err.println("   BREVO_API_KEY = xkeysib-xxxxxxxxxxxxxxxx");
            System.err.println("   EMAIL_FROM = tu-email@gmail.com (verificado en Brevo)");
            return;
        }
        
        // Prueba de envío
        String emailPrueba = "cualquier-email@ejemplo.com"; // Funciona con CUALQUIER email
